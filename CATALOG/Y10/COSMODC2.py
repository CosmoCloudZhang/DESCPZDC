...
def main(tag, folder, directory):
    ...
    # Load
    with open(os.path.join(dataset_directory, 'CATALOG/SIMULATE.yaml'), 'r') as file:
        simulation_list = numpy.sort(yaml.safe_load(file)['healpix_pixels'])
    ...
    for value in simulation_list:
        ...
        # Load
        with h5py.File(os.path.join(dataset_directory, 'CATALOG/SIMULATION_{}.hdf5'.format(value)), 'r') as file:
    ...
    # Save
    with h5py.File(os.path.join(catalog_folder, '{}/COSMODC2.hdf5'.format(tag)), 'w') as file:
...
if __name__ == '__main__':
    # Input
    PARSE = argparse.ArgumentParser(description='CosmoDC2')
